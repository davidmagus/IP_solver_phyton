...
Sample = [5, 5 ,5, 10, 50]              #n, m, k, size = 10, Percentage_of_wrong = 50. Extra parameters can be set with rewriting the function call in row 51
...
#setting up things related to the modell
both = False
smart = True
if model == "both":
    both = True
...
if not both: 
    ...
    for M in Matrices:
        id_value, A, b, k = M
        if smart:
            Solution, _, _ = Smart_solver(A, b, k,checkeq= checkeq)
        else:
            Solution, _, _ = IP_solver(A, b, k,checkeq= checkeq)
        if long:
